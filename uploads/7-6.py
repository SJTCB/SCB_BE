...
mlp_sgd = Sequential()
mlp_sgd.add(Dense(units=1024, activation='relu', input_shape=(784,)))
mlp_sgd.add(Dense(units=512, activation='relu'))
mlp_sgd.add(Dense(units=512, activation='relu'))
mlp_sgd.add(Dense(units=10, activation='softmax'))

mlp_sgd.compile(loss='categorical_crossentropy', optimizer=SGD(learning_rate=0.01), metrics=['accuracy'])
